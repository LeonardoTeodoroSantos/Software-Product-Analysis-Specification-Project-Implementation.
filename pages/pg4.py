...
register_page(__name__, name="Prova", path='/', external_stylesheets=[dbc.themes.YETI])
...
@callback(
    ...
    [Input("list_years", "value"), Input("button", "children")]
)
...
@callback(
    ...
    [Input("bar-graph", "clickData"), Input("button", "n_clicks")]
)
def update_location(click_data, n_clicks):
    changed_id = [p["prop_id"] for p in dash.callback_context.triggered][0]
    if click_data is not None and changed_id != "button.n_clicks":
